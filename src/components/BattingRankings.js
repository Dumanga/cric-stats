...
const BattingRankings = () => {
  ...
  const getMovementIcon = (movement) => {
    if (movement > 0)
      return <ArrowUpwardIcon sx={{ color: "#22c55e", fontSize: "1rem" }} />;
    if (movement < 0)
      return <ArrowDownwardIcon sx={{ color: "#ef4444", fontSize: "1rem" }} />;
    return null;
  };

  const avgTwoDp = (runs, innings) =>
    innings > 0 ? (runs / innings).toFixed(2) : "0.00";
                ...
                <TableCell>
                  ...
                  >
                    <Avatar
                      src={
                        row.avatar
                          ? `${process.env.PUBLIC_URL}${row.avatar}`
                          : null
                      }
                      alt={row.playerName}
                      sx={{
